...
Resources in Cloud de Confiance by S3NS are organized into a
...
Cloud de Confiance by S3NS projects to return might be too large. To fix this, apply the
...
In the toolbar of the Cloud de Confiance console, click
**Search for resources, docs, products, and more** and type `apps-script`.

[Go to Cloud de Confiance console](https://console.cloud.s3nscloud.fr/welcome) 
...
Cloud de Confiance console. However, the resource will be returned in a search using the
API or Google Cloud CLI that specifies the resource's parent. This disparity
between the Cloud de Confiance console and other methods is a common source of confusion
...
visible when viewed in the Cloud de Confiance console.
...
only resource that appears in the Cloud de Confiance console is the Organization.
