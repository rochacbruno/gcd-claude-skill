...
Source: https://documentation.s3ns.fr/docs/quotas/gcloud-cli-examples
...
quotas values and specify quota preferences for some Cloud de Confiance by S3NS APIs and
services.
...
value. You can find the quota ID using the Cloud de Confiance console, the
gcloud CLI, client libraries, or the REST API. This section shows how
to find the quota ID using either the Cloud de Confiance console or gcloud CLI.
...
To find your project ID using the Cloud de Confiance console, navigate to the
Resource Manager page:

[Go to Resource Manager](https://console.cloud.s3nscloud.fr/cloud-resource-manager)
...
case Cloud de Confiance by S3NS needs more information before additional quota can be granted.
...
and interact with Cloud de Confiance by S3NS resources, while a resource project is the
...
--iam-account= SA_NAME @ CLIENT_PROJECT_ID .s3ns.iam.gserviceaccount.com
```
...
- ` SA_NAME @ CLIENT_PROJECT_ID .s3ns.iam.gserviceaccount.com`:
...
--member="serviceAccount: SA_NAME @ CLIENT_PROJECT_ID .s3ns.iam.gserviceaccount.com" \
