...
specific operations on the following Cloud de Confiance by S3NS resources:
...
The Cloud de Confiance Organization Policy Service gives you centralized, programmatic
...
Cloud de Confiance resources and descendants of those resources in the
[Cloud de Confiance by S3NS resource hierarchy][op-hierarchy]. You can enforce organization
...
for various Cloud de Confiance services. However, if you want more granular,
...
on a folder, Cloud de Confiance enforces the policy on all projects in the
...
**When initiating quota changes in the Cloud de Confiance console**: Quota value
changes initiated using the Cloud de Confiance console aren't subject to custom
organization policy enforcement because the Cloud de Confiance console does not
...
- 
In the **Resource type** box, select the name of the Cloud de Confiance REST resource
...
- 
` RESOURCE_NAME `: the fully qualified name of the Cloud de Confiance
...
policies in your list of Cloud de Confiance by S3NS organization policies.
...
applying that organization policy to a Cloud de Confiance by S3NS resource.
...
After you apply the policy, wait for about two minutes for Cloud de Confiance to
