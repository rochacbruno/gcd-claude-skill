...
Source: https://documentation.s3ns.fr/docs/authentication/application-default-credentials
...
(IdP) to authenticate and authorize users to access Cloud de Confiance
...
Cloud de Confiance resources. For more information, see
...
gcloud CLI uses to authenticate to Cloud de Confiance. For more
...
Many Cloud de Confiance services let you attach a service account that can be
used to provide credentials for accessing Cloud de Confiance APIs. If ADC does
...
for finding credentials in a production environment on Cloud de Confiance. To
