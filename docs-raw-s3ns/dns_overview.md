...
Source: https://documentation.s3ns.fr/dns/docs/overview
...
Cloud de Confiance offers inbound and outbound DNS forwarding for private zones.
...
VPC network. For information about how Cloud de Confiance
...
Cloud de Confiance console](https://console.cloud.s3nscloud.fr/iam-admin).
For users to be authorized to make changes, they must have the
DNS Administrator role (`roles/dns.admin`) in the Permissions
section of the Cloud de Confiance console. The DNS Reader role (`roles/dns.reader`)
...
- Learn about [Cloud DNS differences in Cloud de Confiance versus Google Cloud](/dns/docs/tpc-differences).
