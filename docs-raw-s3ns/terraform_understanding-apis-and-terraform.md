# Understanding Cloud de Confiance by S3NS APIs and Terraform

Source: https://documentation.s3ns.fr/docs/terraform/understanding-apis-and-terraform
Last updated: 2026-08-26

Some or all of the information on this page might not apply to Cloud de Confiance by S3NS. See [Differences from Google Cloud](/docs/iac/tpc-differences) for more details.
...
- On this page 
- [ Public versus private Cloud de Confiance by S3NS APIs ](#pub-vs-priv-gcp-apis)
...
This guide aims to clarify how Terraform interacts with Cloud de Confiance by S3NS APIs
...
effectively managing your Cloud de Confiance by S3NS resources with Terraform and avoiding
common pitfalls.

## Public versus private Cloud de Confiance by S3NS APIs 

Cloud de Confiance by S3NS services expose various APIs that allow applications and tools
(like Terraform) to interact with and manage resources. These APIs broadly fall
into two categories:

### Public APIs 

**Purpose:** These are the primary interfaces for customers and tools to create, configure, and manage Cloud de Confiance by S3NS resources (e.g., Compute Engine instances, Cloud Storage buckets, BigQuery datasets).
...
**Purpose:** These APIs are internal to Cloud de Confiance by S3NS services, used by Google itself for the internal operation, orchestration, and provisioning of its managed services. They expose functionalities that are not meant for direct customer interaction or management.
...
resource" is fundamental to using Terraform effectively with Cloud de Confiance by S3NS.

### Enabling an API

- **What it means:** When you "enable an API" in Cloud de Confiance by S3NS, you are activating a specific Cloud de Confiance by S3NS service for your project. This grants your project the necessary permissions and access to use the functionalities of that service and create resources managed by it.

**Terraform context:** In Terraform, this is typically done using the `google_project_service` resource. This resource verifies that a specified public API (e.g., `compute.googleapis.com`) is enabled for your Cloud de Confiance by S3NS project.
...
internal Cloud de Confiance by S3NS API, you **don't** need to explicitly enable it using
...
Cloud de Confiance by S3NS services. They are automatically managed by Google and are not
...
use the associated Cloud de Confiance by S3NS service (e.g., Managed Service for Apache Spark will function
...
**Focus on Public APIs:** When managing Cloud de Confiance by S3NS resources with Terraform,
...
By understanding the clear distinction between public and private Cloud de Confiance by S3NS
APIs, and the specific roles of "enabling" APIs versus "importing" resources in
Terraform, you can effectively manage your Cloud de Confiance by S3NS infrastructure. don't
attempt to explicitly manage or import private Cloud de Confiance by S3NS APIs; they are
