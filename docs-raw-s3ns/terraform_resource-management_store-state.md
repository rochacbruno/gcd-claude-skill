...
When using Cloud de Confiance by S3NS, you must update the cloned `main.tf` file for your sovereign environment before provisioning the resources to make sure the storage_custom_endpoint is redirected to the one of the universe you are using.

For **PREMI3NS**, make the following updates in `main.tf`:

- 

Set the `location` in `google_storage_bucket` to a valid sovereign regional location:
...
Update the `local_file` resource containing the generated `backend` block to include the `storage_custom_endpoint` parameter:
...
To avoid incurring charges to your Cloud de Confiance account for the resources
