...
Cloud de Confiance console. You can also set up [alerts](#alerts) to monitor changes
...
available in the Cloud de Confiance console. To access quota adjuster
settings at the folder or organization level, use the [Cloud Quotas API](/docs/quotas/reference/rest)
([Preview](https://documentation.s3ns.fr/products#product-launch-stages)) or the
...
Quota adjuster availability depends on your Cloud de Confiance by S3NS project and
is only available for some Cloud de Confiance by S3NS quotas.
...
Cloud de Confiance by S3NS quotas. A Cloud de Confiance by S3NS service might support the
...
When a Cloud de Confiance by S3NS service adds or expands
...
|
| 
Cloud de Confiance by S3NS Memorystore for Redis API | 
Total Redis capacity (GB) per region | 
|
| 
Cloud de Confiance Memorystore for Redis API | 
...
Note: To see the full list of available dimensions, expand this entry in the Cloud de Confiance console.
...
[Go to Policies](https://console.cloud.s3nscloud.fr/monitoring/alerting/policies)
...
adjuster will be denied. This sometimes occurs when Cloud de Confiance by S3NS is unable to
