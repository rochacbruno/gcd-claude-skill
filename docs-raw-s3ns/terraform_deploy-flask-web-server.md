...
- [ Run a web server on Cloud de Confiance ](#run-web-server)
...
- Use Terraform to create a VM in Cloud de Confiance.
...
[
Compute Engine](https://documentation.s3ns.fr/compute/all-pricing)
...
[
Go to the IAM page](https://console.cloud.s3nscloud.fr/iam-admin/iam)
...
[Enable the API](https://console.cloud.s3nscloud.fr/apis/enableflow?apiid=compute.googleapis.com)
...
describe all of the Cloud de Confiance resources to be created in the project.
...
The sample code sets the Cloud de Confiance zone to `us-west1-a`. You can change
...
Terraform calls Cloud de Confiance APIs to set up the new VM. Check the
[VM instances page](https://console.cloud.s3nscloud.fr/compute/instances) to
see the new VM.

## Run a web server on Cloud de Confiance
...
Go to the [VM Instances page](https://console.cloud.s3nscloud.fr/compute/instances).
...
port 5000 open. Cloud de Confiance lets you open ports to traffic by using
firewall rules.
