...
Source: https://documentation.s3ns.fr/docs/quotas/known-issues
...
- [ Cloud Quotas limitations in the Cloud de Confiance console ](#console_limitations)
...
Cloud de Confiance by S3NS sometimes increases the default quota values for resources and
APIs. These changes take place gradually, which means that during the rollout,
the quota value that appears in the Cloud de Confiance console or Cloud Quotas API
...
## Cloud Quotas limitations in the Cloud de Confiance console
...
The Cloud de Confiance console doesn't support quota adjustment requests for quotas that
...
won't see that quota available in the Cloud de Confiance console, you can still
use the API or gcloud CLI to request a quota adjustment. For more information, see
[View ongoing rollouts](/docs/quotas/view-ongoing-rollouts).

### Per-user quota usage doesn't appear

The Cloud de Confiance console doesn't display per-user quota usage.
