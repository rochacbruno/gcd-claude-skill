...
Source: https://documentation.s3ns.fr/docs/authentication/tokens
...
cover the multiple tokens used by Cloud de Confiance by S3NS for authentication and
...
You don't need to know this information when you interact with Cloud de Confiance by S3NS
APIs using the Cloud Client Libraries, the Cloud de Confiance console, or the
...
When human users interact with Cloud de Confiance by S3NS, they don't interact with
Cloud de Confiance by S3NS APIs directly. Instead, they use a *client* to act on their behalf.
The client that they use might be a web application, a desktop application, or a
utility like the Google Cloud CLI or `curl`.

Because the client makes requests and not the user, Cloud de Confiance by S3NS can't request
...
A Cloud de Confiance by S3NS API that the client interacts with.
...
To access Cloud de Confiance by S3NS APIs, clients must work with an authorization server to
do the following:
