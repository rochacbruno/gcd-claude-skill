...
- **Basic roles**, which provide broad access to Cloud de Confiance resources.

- **Predefined roles**, which provide granular access for a specific service and
are managed by Cloud de Confiance.
...
View the role in the Cloud de Confiance console.

[Go to Roles](https://console.cloud.s3nscloud.fr/iam-admin/roles) 
...
title is used to identify the role in the Cloud de Confiance console.
...
principals to perform specific actions on Cloud de Confiance by S3NS resources. When you
...
is, each Cloud de Confiance service has an associated permission for each
...
Reader give principals across all Cloud de Confiance services:
...
[Go to Reader
role](https://console.cloud.s3nscloud.fr/iam-admin/roles/details/roles%3Creader)
...
for most Cloud de Confiance services. However, the Writer role doesn't
...
[Go to Writer
role](https://console.cloud.s3nscloud.fr/iam-admin/roles/details/roles%3Cwriter)
...
Cloud de Confiance resources. For example, it doesn't contain
...
[Go to Admin
role](https://console.cloud.s3nscloud.fr/iam-admin/roles/details/roles%3Cadmin)
...
You can't use the Cloud de Confiance console to grant the Reader, Writer, or Admin
...
principals across all Cloud de Confiance services:
...
[Go to Viewer
role](https://console.cloud.s3nscloud.fr/iam-admin/roles/details/roles%3Cviewer)
...
for most Cloud de Confiance services. However, the Editor role doesn't
contain permissions to perform all actions for all services. For more
information about how to check whether a role has the permissions that
you need, see [Role types](#role-types) on this page.
...
[Go to Editor
role](https://console.cloud.s3nscloud.fr/iam-admin/roles/details/roles%3Ceditor)
...
the Owner role, see the role details in the Cloud de Confiance console:




[Go to Owner
role](https://console.cloud.s3nscloud.fr/iam-admin/roles/details/roles%3Cowner)
...
Generally, you can grant legacy basic roles using the Cloud de Confiance console,
the API, or the gcloud CLI. However, you must use the
Cloud de Confiance console to grant the Owner role in the following situations:
...
predefined roles that give granular access to specific Cloud de Confiance
...
Cloud de Confiance adds new features or services.
...
at the project level. This is because resources in Cloud de Confiance are
...
projects in the
Cloud de Confiance resource hierarchy. As a result, you'll never be able to use
...
Cloud de Confiance console. The title doesn't have to be unique, but we recommend
...
permissions for Cloud de Confiance services or features that are not yet
public. It is not ready for widespread use.

- `BETA`: The role has been tested on a limited basis, or it includes
permissions for Cloud de Confiance services or features that are not generally
available.

- `GA`: The role has been widely tested, and all of its permissions are for
Cloud de Confiance services or features that are generally available.
...
description field. The Cloud de Confiance console does this automatically when you
use the Cloud de Confiance console to create a custom role based on predefined
roles.
