...
Source: https://documentation.s3ns.fr/logging/docs/audit
...
- On this page ** 
- [ Cloud de Confiance by S3NS services producing audit logs ](#services)
...
Cloud de Confiance by S3NS services write audit logs that record administrative activities
and resource access. You can use Cloud Audit Logs to answer
"who did what, where, and when?" within your Cloud de Confiance resources
with the same level of transparency as in on-premises environments. These
audit logs help your security, auditing, and compliance teams monitor
Cloud de Confiance data and systems for vulnerabilities or data misuse.

## Cloud de Confiance by S3NS services producing audit logs 

For a list of Cloud de Confiance services that provide audit logs, see
[Cloud de Confiance by S3NS services with audit logs](/logging/docs/audit/services). All
Cloud de Confiance services will eventually provide audit logs.
...
Cloud de Confiance project, folder, and organization:
...
[Cloud de Confiance by S3NS services with audit logs](/logging/docs/audit/services).
...
that can be accessed without logging into a Cloud de Confiance account don't
generate audit logs. This helps protect end-user identities and information.

Except for BigQuery, Data Access audit logs are disabled by default
because they can generate large volumes of data. To record Data Access
audit logs for Cloud de Confiance services other than
BigQuery, you must explicitly enable them. Data Access audit logs
are written to the Cloud de Confiance project whose data is accessed.
Enabling these logs might result in your Cloud de Confiance project being
charged for additional log usage. For instructions on enabling and
...
System Event audit logs are log entries written by Cloud de Confiance systems
and managed service agents
...
Policy Denied audit logs are log entries written when a Cloud de Confiance by S3NS service
...
Cloud de Confiance project is charged for log storage. You can't disable Policy
...
To understand how to read and interpret audit log entries, and to view a sample
...
Resource identifiers indicating the Cloud de Confiance project or
other Cloud de Confiance entity that owns the audit logs.
...
Cloud de Confiance resource. The caller's identity is held in the
...
Cloud de Confiance by S3NS service to another, the `callerIp` is redacted to "private".
...
- For a caller from a Compute Engine VM with an external IP address, the
...
multi-service flows, such as Cloud Logging calling Cloud KMS,
the service propagates the immediate caller's origin unless
explicit identity delegation is configured.
As a result, an external address might appear instead of "private".

- For a caller from a Compute Engine VM without an external IP address, if
...
of the Cloud de Confiance project, folder, billing account, or
...
Cloud de Confiance console, the Google Cloud CLI, or the Logging API.
...
In the Cloud de Confiance console, you can use the Logs Explorer
to retrieve your audit log entries for your Cloud de Confiance project, folder,
or organization:
...
[Go to **Logs Explorer**](https://console.cloud.s3nscloud.fr/logs/query)
...
In **Resource type**, select the Cloud de Confiance resource whose
...
that type available in the Cloud de Confiance project, folder, or
organization.
...
To read your Cloud de Confiance project-level audit log entries, run
...
Cloud de Confiance project, folder, and organization, Logging
...
different project, then those logs don't pass through the destination
project's `_Default` or `_Required` sink. Consequently, the logs aren't stored
in the destination project.
To store these logs, create a log sink in the destination project.
...
Cloud Logging buckets at the Cloud de Confiance project level or to supported
...
For information about routing audit log entries for a Cloud de Confiance
organization, folder, or billing account, and for their children, see
