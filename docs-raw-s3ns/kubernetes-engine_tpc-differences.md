...
](https://documentation.s3ns.fr/kubernetes-engine/docs/concepts/kubernetes-engine-overview)
...
Cloud de Confiance and Google Cloud versions of GKE.
...
There are some differences between the Cloud de Confiance version of GKE and
...
*(Versions between 1.36.0-gke.1266000 and 1.36.0-gke.2684000 only)* Specify the `custom-endpoint` mount option in the volume's `mountOptions`, either in a Pod's ephemeral storage volume or in a PersistentVolume, to point to the Cloud Storage API endpoint for your environment: `storage.s3nsapis.fr:443`.
...
- **CLI format**: `custom-endpoint=storage.s3nsapis.fr:443`

- **Configuration file format**: `gcs-connection:custom-endpoint:storage.s3nsapis.fr:443`
...
Workload identity pool domains (and the Kubernetes principal identifiers that use them) are specified slightly differently in Google Cloud and Cloud de Confiance by S3NS, using `s3ns.svc.id.goog` rather than `svc.id.goog`. So, for example, when specifying a Kubernetes ServiceAccount using Workload Identity Federation for GKE in an [IAM policy](/kubernetes-engine/docs/concepts/workload-identity#kubernetes-resources-iam-policies), you use `.../ PROJECT_ID .s3ns.svc.id.goog/subject/ns/ NAMESPACE /sa/ KUBERNETES_SERVICE_ACCOUNT `.
...
In Cloud de Confiance, service agents (universe-managed service accounts) are provisioned **Just-In-Time (JIT)** when you create your first resource for a service, rather than when the API is enabled.
...
When using the GKE Ingress controller, the global Google Cloud external application load balancer (`gxlb`) is not used. In Cloud de Confiance by S3NS, a regional external load balancer (`rxlb`) is used.
| 
|
...
- Centralized management through the Config Sync dashboard in the Cloud de Confiance console.

- Integrated dashboards and Cloud Monitoring metrics in the Cloud de Confiance console. You can use local Prometheus scraping (port 8675) for metrics and alerting instead.
...
- 
[Learn more about Autopilot.](https://documentation.s3ns.fr/kubernetes-engine/docs/concepts/autopilot-overview")
