...
Cloud de Confiance by S3NS's infrastructure.
...
[Get started in Cloud de Confiance console](https://console.cloud.s3nscloud.fr/kubernetes/overview/get-started)
...
Cloud de Confiance manages both the
...
GKE, Cloud de Confiance manages the
control plane and system components for you. In Autopilot mode, which
is the recommended way to run GKE,
Cloud de Confiance also manages your worker nodes.
Cloud de Confiance automatically upgrades
...
GKE in Cloud de Confiance has one mode of operation,
Autopilot, which offers a fully managed experience. GKE
Standard clusters are unavailable in Cloud de Confiance.

## What's next

- Learn about [GKE differences in Cloud de Confiance versus Google Cloud](/kubernetes-engine/docs/tpc-differences).
...
- [Design and optimize your GKE cluster with Gemini](/kubernetes-engine/docs/design-with-gemini).