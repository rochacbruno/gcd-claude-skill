...
- [ Cloud de Confiance default permissions ](#gcp)
...
- [ Integrating with Cloud de Confiance services ](#integration)

- [ Default service accounts for Cloud de Confiance by S3NS services ](#gcp-permissions)
...
### Cloud de Confiance default permissions

By default, the following permissions apply to Cloud de Confiance CI/CD services
...
If all your services are in the same Cloud de Confiance by S3NS project and the default
...
Traditionally, applications running outside Cloud de Confiance use
[service account keys](/iam/docs/service-account-creds#key-types)
to access Cloud de Confiance resources. However, service account keys are
...
[Allow your external workload to access Cloud de Confiance by S3NS resources](/iam/docs/workload-identity-federation-with-other-providers#access).
...
You can grant roles on the Cloud de Confiance by S3NS project or the Artifact Registry
repository.

### Predefined Artifact Registry roles

IAM provides [predefined roles](/iam/docs/roles-permissions)
that grant access to specific Cloud de Confiance resources.
...
[Open
the IAM page](https://console.cloud.s3nscloud.fr/iam-admin/iam) 
...
[Open the Repositories page](https://console.cloud.s3nscloud.fr/artifacts)
...
repository `my-repo` in the location `--u-france-east1`, run:
...
To revoke public access to `my-repo` in the location `--u-france-east1`, run:
...
Project administrators can create tags for resources across Cloud de Confiance
...
## Integrating with Cloud de Confiance services

For most Cloud de Confiance service accounts, configuring access to a registry
only requires granting the appropriate IAM roles.

### Default service accounts for Cloud de Confiance by S3NS services

Cloud de Confiance services such as Google Kubernetes Engine use a
...
- The Cloud de Confiance by S3NS service is in a different project than Artifact Registry.
...
email address for the service account includes the Cloud de Confiance
...
suffix **@developer.s3ns-system.iam.gserviceaccount.com**.
...
Cloud de Confiance project where Artifact Registry is running.
