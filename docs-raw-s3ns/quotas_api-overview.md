...
Source: https://documentation.s3ns.fr/docs/quotas/api-overview
...
quotas that need to be increased for every new Cloud de Confiance by S3NS project, you can
use the Cloud Quotas API to automate this in the creation logic of your project.
Quota adjustment requests are subject to Cloud de Confiance by S3NS approval.

- 

**Serve customer quota requests**: If you are a SaaS provider integrated with
Cloud de Confiance by S3NS, you might receive quota increase requests through a
customer-facing portal other than the Cloud de Confiance console. These requests must be
forwarded to Cloud de Confiance by S3NS for processing. The Cloud Quotas API can automatically
...
- ` PROJECT `: the ID or number of your Cloud de Confiance by S3NS
project.
...
Approved quota increase requests by Cloud de Confiance by S3NS.

- 

Changes to quotas initiated by Cloud de Confiance by S3NS.

The ability to delete a `QuotaPreference` is not supported. However, you can set
a preferred quota value lower than the Cloud de Confiance by S3NS approved value to add further guardrails.
...
the quotas defined by Cloud de Confiance by S3NS services and any fulfilled quota
...
The `QuotaAdjusterSettings` resource ([Preview](https://documentation.s3ns.fr/products#product-launch-stages))
