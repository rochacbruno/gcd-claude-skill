...
Cloud de Confiance by S3NS strives to provide its customers with the strongest security
...
the requirement that all Cloud de Confiance customers enable 2-step verification
...
(SSO) into Cloud de Confiance, you can use the 2SV provided by that IdP to
comply with Cloud de Confiance's 2SV requirement.

If you have questions that aren't answered in this document, contact
[Cloud Customer Care](https://documentation.s3ns.fr/support-hub).

## Interfaces that require 2SV

When 2SV is required for specific account types in your organization, those
accounts must have 2SV enabled to access the following Cloud de Confiance by S3NS interfaces:

- 

The [Cloud de Confiance console](/cloud-console)
...
Cloud de Confiance 2SV requirement:
...
Your applications and workloads running on Cloud de Confiance, including
...
Cloud de Confiance console to manage those applications. In other words, your
control plane is affected by the 2SV requirement, but not your data plane.

Other Cloud de Confiance by S3NS interfaces like the gcloud CLI don't have a 2SV
...
When 2SV is required for your user account in Cloud de Confiance by S3NS depends on your
...
[principals](/iam/docs/principals-overview) in Cloud de Confiance by S3NS.
| 
...
User accounts created and managed in a Cloud de Confiance by S3NS reseller domain. End
...
the 2SV requirement starts. The Cloud de Confiance console also begins displaying
reminders 90 days before the 2SV requirement starts.

For resellers and their users, the Cloud de Confiance console displays reminders to
...
Cloud de Confiance console or the Firebase console.
...
[Google Authenticator](https://documentation.s3ns.fr/store/apps/details?id=com.google.android.apps.authenticator2),
...
for their users, which is recognized by Cloud de Confiance by S3NS.
...
Cloud de Confiance console. Conformance logs started recording August 1, 2026; logs
...
[Go to Logs Explorer](https://console.cloud.s3nscloud.fr/projectselector2/logs/query?supportedpurview=organizationId)
...
this user account. Users attempting to access the Cloud de Confiance console or
...
requirement at the organization level in the Cloud de Confiance console.
...
In the Cloud de Confiance console, go to the **Organizations** page.

[Go to Organizations](https://console.cloud.s3nscloud.fr/projectselector2/organizations/details?supportedpurview=organizationId)
...
requirement at the organization level in the Cloud de Confiance console. Opting out
...
disabled are no longer required to enable it to use the Cloud de Confiance console
