...
If your project exceeds its maximum quota value while using a service, Cloud de Confiance by S3NS
returns an error based on how you accessed the service:

- If you exceed a quota value with an API request, Cloud de Confiance by S3NS returns an HTTP
...
- If you exceeded a quota value with an HTTP/REST request, Cloud de Confiance by S3NS returns an
HTTP `429 TOO MANY REQUESTS` status code.

- If you exceed a quota for Compute Engine, Cloud de Confiance by S3NS typically returns an
...
- If you exceeded a quota value using [gRPC](https://grpc.io), Cloud de Confiance by S3NS returns a `ResourceExhausted`
...
Cloud de Confiance by S3NS sometimes changes the default quota values for resources
and APIs. These changes take place gradually, which means that during the
rollout of a new default quota, the quota value that appears in the Cloud de Confiance console
...
and future value are correct, even if what appears in the Cloud de Confiance console
is different.
...
[**IAM & Admin  > Quotas & System Limits**](https://console.cloud.s3nscloud.fr/quotas?project=_)
